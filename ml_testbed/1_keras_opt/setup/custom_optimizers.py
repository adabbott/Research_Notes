...
# code taken from ncullen93 https://github.com/keras-team/keras/issues/5085 
#opt = ScipyOpt(model=model,x=xtrain,y=ytrain, nb_epoch=500) 
...
class ScipyOpt(Optimizer):
    ...
    def get_train_fn(self, params, loss):
        ...
        fn = K.function(inputs=[], outputs=outs)# **givens)#, **{self.model.model.inputs[0] : self.x, self.model.model.targets[0]: self.y, self.model.model.sample_weights[0]  : np.ones((self.x.shape[0],), dtype=np.float32),K.learning_phase(): np.uint8(1)})
        def train_fn(theta):
